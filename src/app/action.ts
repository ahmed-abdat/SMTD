...
export const getMarketPlaceProducts = async (collectionName: string , limited : boolean = true) => {
  ...
  try {
    ...
    // If the fetched product count is less than the limit, it means there are no more products
...
export const getHealthProducts = async (categoryId: string , limited : boolean = true) => {
  const numberOfproducts = limited ? 10 : 60;
  const collectionRef = collection(
    firestore,
    `health-products/${categoryId}/categoryProduct`
  );
  const q = query(collectionRef, orderBy("createdAt", "desc"), limit(numberOfproducts));
  try {
    const snapshot = await getDocs(q);
    let products: Product[] = [];
    snapshot.forEach((doc) => {
