...
export default function ImpactSection({ locale }: { locale: string }) {
  const t = useTranslations('ImpactSection')
  const stats = t.raw('stats') as Array<{ value: number; label: string; suffix: string }>
    ...
    <section className="py-24 bg-primary-darkGreen text-primary-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-bold mb-4 text-primary-yellow">{t('title')}</h2>
              ...
              <Card className="bg-primary-forestGreen border-primary-limeGreen hover:bg-primary-darkGreen transition-colors duration-300">
                <CardContent className="p-6 text-center">
                  <div className="text-5xl font-bold mb-2 text-primary-yellow">
                    {counts[index].toLocaleString(locale)}{stat.suffix}
                  </div>
                  <div className="text-sm text-primary-lightGreen">{stat.label}</div>
                </CardContent>
              </Card>
            </motion.div>
          ))}
